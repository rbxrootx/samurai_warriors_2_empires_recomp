...
set "LOG=%ROOT%runtime.gpu-trace.log"
set "TRACE_ROOT=%ROOT%traces"
...
set "TRACE_ARGS=--trace_gpu_stream=true --trace_gpu_prefix=%TRACE_ROOT%"
...
if exist "%MOD_ROOT%" (
  "%EXE%" "--game_data_root=%GAME_ROOT%" "--mod_data_root=%MOD_ROOT%" "--log_file=%LOG%" --log_level=info %INPUT_ARGS% %TRACE_ARGS%
) else (
  "%EXE%" "--game_data_root=%GAME_ROOT%" "--log_file=%LOG%" --log_level=info %INPUT_ARGS% %TRACE_ARGS%
)
