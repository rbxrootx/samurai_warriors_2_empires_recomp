...
set "LOG=%ROOT%runtime.native-events.log"
...
set "NATIVE_EVENT_ARGS=--native_render_events=true --native_render_events_path=%EVENT_PATH% --native_render_event_limit=50000 --sw2e_native_renderer=true"
...
if exist "%MOD_ROOT%" (
  "%EXE%" "--game_data_root=%GAME_ROOT%" "--mod_data_root=%MOD_ROOT%" "--log_file=%LOG%" --log_level=info %INPUT_ARGS% %NATIVE_EVENT_ARGS%
) else (
  "%EXE%" "--game_data_root=%GAME_ROOT%" "--log_file=%LOG%" --log_level=info %INPUT_ARGS% %NATIVE_EVENT_ARGS%
)
