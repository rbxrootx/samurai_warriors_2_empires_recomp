...
constexpr uint32_t kNativeReplayNoRestartOverride = std::numeric_limits<uint32_t>::max();
...
bool IsNativeReplaySharedShaderSkinProjectionShader(uint64_t vertex_shader_hash) {
  return vertex_shader_hash == 0xED8D12865D27DEBFull ||
         ...
         vertex_shader_hash == 0x2E01DF902B14A323ull ||
...
bool IsNativeReplay6B72ModelVertexFetch(const VertexFetchSummary& fetch) {
  const uint32_t attribute_count =
      std::min(fetch.attribute_summary_count,
               rex::graphics::native_render::kMaxVertexAttributeSummariesPerFetch);
  if (fetch.stride_words != 9 || attribute_count != 4) {
    return false;
  }

  const VertexAttributeSummary& position_attribute = fetch.attributes[0];
  const VertexAttributeSummary& normal_attribute = fetch.attributes[1];
  const VertexAttributeSummary& packed_attribute = fetch.attributes[2];
  const VertexAttributeSummary& texcoord_attribute = fetch.attributes[3];
  return position_attribute.data_format == kXenosVertexFormat32_32_32Float &&
         ...
         position_attribute.result_storage_index == 6 &&
         position_attribute.result_swizzle == 2696 &&
         normal_attribute.data_format == kXenosVertexFormat32_32_32Float &&
         normal_attribute.offset_words == 3 &&
         normal_attribute.result_storage_target == 1 &&
         normal_attribute.result_storage_index == 4 &&
         normal_attribute.result_swizzle == 2184 &&
         packed_attribute.data_format == kXenosFormat8888 &&
         packed_attribute.offset_words == 6 &&
         packed_attribute.result_storage_target == 1 &&
         packed_attribute.result_storage_index == 1 &&
         packed_attribute.result_swizzle == 90 &&
         texcoord_attribute.data_format == kXenosVertexFormat32_32Float &&
         texcoord_attribute.offset_words == 7 &&
...
bool IsNativeReplay2E01ModelVertexFetch(const VertexFetchSummary& fetch) {
  const uint32_t attribute_count =
      std::min(fetch.attribute_summary_count,
               rex::graphics::native_render::kMaxVertexAttributeSummariesPerFetch);
  if (fetch.stride_words != 7 || attribute_count != 3) {
    return false;
  }
...
bool IsNativeReplayED8DSharedSkinVertexFetch(const VertexFetchSummary& fetch) {
  const uint32_t attribute_count =
      std::min(fetch.attribute_summary_count,
               rex::graphics::native_render::kMaxVertexAttributeSummariesPerFetch);
  if (fetch.stride_words != 12 || attribute_count != 4) {
    return false;
  }

  const VertexAttributeSummary& position_attribute = fetch.attributes[0];
  const VertexAttributeSummary& weight_attribute = fetch.attributes[1];
  const VertexAttributeSummary& packed_attribute = fetch.attributes[2];
  const VertexAttributeSummary& texcoord_attribute = fetch.attributes[3];
  return position_attribute.data_format == kXenosVertexFormat32_32_32Float &&
         position_attribute.offset_words == 0 &&
         ...
         weight_attribute.data_format == kXenosVertexFormat32_32Float &&
         weight_attribute.offset_words == 3 &&
         weight_attribute.result_storage_target == 1 &&
         weight_attribute.result_storage_index == 0 &&
         weight_attribute.result_swizzle == 2060 &&
         packed_attribute.data_format == kXenosFormat8888 &&
         packed_attribute.offset_words == 5 &&
         packed_attribute.result_storage_target == 1 &&
         packed_attribute.result_storage_index == 3 &&
         packed_attribute.result_swizzle == 83 &&
         texcoord_attribute.data_format == kXenosVertexFormat32_32Float &&
         texcoord_attribute.offset_words == 10 &&
         texcoord_attribute.result_storage_target == 1 &&
         texcoord_attribute.result_storage_index == 2 &&
         texcoord_attribute.result_swizzle == 2312;
}

bool IsNativeReplay6E10ModelVertexFetch(const VertexFetchSummary& fetch) {
  const uint32_t attribute_count =
      std::min(fetch.attribute_summary_count,
               rex::graphics::native_render::kMaxVertexAttributeSummariesPerFetch);
  ...
  return position_attribute.data_format == kXenosVertexFormat32_32_32Float &&
         position_attribute.offset_words == 0 &&
         position_attribute.result_storage_target == 1 &&
         position_attribute.result_storage_index == 9 &&
         position_attribute.result_swizzle == 2696 &&
         ...
         packed_index_attribute.result_storage_index == 1 &&
         packed_index_attribute.result_swizzle == 2276 &&
         normal_attribute.data_format == kXenosVertexFormat32_32_32Float &&
         normal_attribute.offset_words == 4 &&
         normal_attribute.result_storage_target == 1 &&
         normal_attribute.result_storage_index == 3 &&
         normal_attribute.result_swizzle == 1092 &&
         ...
         packed_material_attribute.result_storage_index == 2 &&
         packed_material_attribute.result_swizzle == 90 &&
         texcoord_attribute.data_format == kXenosVertexFormat32_32Float &&
         texcoord_attribute.offset_words == 8 &&
         texcoord_attribute.result_storage_target == 1 &&
         texcoord_attribute.result_storage_index == 1 &&
         texcoord_attribute.result_swizzle == 2312;
}

bool IsNativeReplay83BDModelVertexFetch(const VertexFetchSummary& fetch) {
  const uint32_t attribute_count =
      std::min(fetch.attribute_summary_count,
               rex::graphics::native_render::kMaxVertexAttributeSummariesPerFetch);
  if (fetch.stride_words != 12 || attribute_count != 6) {
    return false;
  }

  const VertexAttributeSummary& position_attribute = fetch.attributes[0];
  const VertexAttributeSummary& weight_attribute = fetch.attributes[1];
  const VertexAttributeSummary& packed_index_attribute = fetch.attributes[2];
  const VertexAttributeSummary& normal_attribute = fetch.attributes[3];
  const VertexAttributeSummary& packed_material_attribute = fetch.attributes[4];
  const VertexAttributeSummary& texcoord_attribute = fetch.attributes[5];
  return position_attribute.data_format == kXenosVertexFormat32_32_32Float &&
         position_attribute.offset_words == 0 &&
         position_attribute.result_storage_target == 1 &&
         position_attribute.result_storage_index == 8 &&
         position_attribute.result_swizzle == 2696 &&
         weight_attribute.data_format == kXenosVertexFormat32_32Float &&
         weight_attribute.offset_words == 3 &&
         weight_attribute.result_storage_target == 1 &&
         weight_attribute.result_storage_index == 5 &&
         weight_attribute.result_swizzle == 2312 &&
         packed_index_attribute.data_format == kXenosFormat8888 &&
         packed_index_attribute.offset_words == 5 &&
         packed_index_attribute.result_storage_target == 1 &&
         packed_index_attribute.result_storage_index == 1 &&
         packed_index_attribute.result_swizzle == 2131 &&
         normal_attribute.data_format == kXenosVertexFormat32_32_32Float &&
         normal_attribute.offset_words == 6 &&
         normal_attribute.result_storage_target == 1 &&
         normal_attribute.result_storage_index == 7 &&
         normal_attribute.result_swizzle == 2184 &&
         packed_material_attribute.data_format == kXenosFormat8888 &&
         packed_material_attribute.offset_words == 9 &&
         packed_material_attribute.result_storage_target == 1 &&
         packed_material_attribute.result_storage_index == 2 &&
         packed_material_attribute.result_swizzle == 90 &&
         texcoord_attribute.data_format == kXenosVertexFormat32_32Float &&
         texcoord_attribute.offset_words == 10 &&
         texcoord_attribute.result_storage_target == 1 &&
         texcoord_attribute.result_storage_index == 3 &&
         texcoord_attribute.result_swizzle == 2312;
}
...
bool IsNativeReplay3094EffectVertexFetch(const VertexFetchSummary& fetch) {
  const uint32_t attribute_count =
      std::min(fetch.attribute_summary_count,
               rex::graphics::native_render::kMaxVertexAttributeSummariesPerFetch);
  if (fetch.stride_words != 8 || attribute_count != 2) {
    return false;
  }

  const VertexAttributeSummary& position_attribute = fetch.attributes[0];
  const VertexAttributeSummary& texcoord_attribute = fetch.attributes[1];
  return position_attribute.data_format == kXenosVertexFormat32_32_32_32Float &&
         position_attribute.offset_words == 0 &&
         position_attribute.result_storage_target == 1 &&
         position_attribute.result_storage_index == 1 &&
         position_attribute.result_swizzle == 1672 &&
         texcoord_attribute.data_format == kXenosVertexFormat32_32Float &&
         texcoord_attribute.offset_words == 4 &&
         texcoord_attribute.result_storage_target == 1 &&
         texcoord_attribute.result_storage_index == 0 &&
         texcoord_attribute.result_swizzle == 2312;
}
...
bool DecodeNativeReplayTexturedVertex(const uint8_t* vertex, uint32_t vertex_stride_bytes,
  ...
  if (IsNativeReplay83BDModelVertexFetch(fetch) && vertex_stride_bytes >= 48) {
    ...
    replay_vertex.z = ReadF32(vertex + 8, fetch.endian);
    replay_vertex.w = 1.0f;
    replay_vertex.u = ReadF32(vertex + 40, fetch.endian);
    replay_vertex.v = ReadF32(vertex + 44, fetch.endian);
    return true;
  }

  if (IsNativeReplayED8DSharedSkinVertexFetch(fetch) && vertex_stride_bytes >= 48) {
  ...
  if (IsNativeReplay6B72ModelVertexFetch(fetch) && vertex_stride_bytes >= 36) {
    replay_vertex.x = ReadF32(vertex + 0, fetch.endian);
    replay_vertex.y = ReadF32(vertex + 4, fetch.endian);
    replay_vertex.z = ReadF32(vertex + 8, fetch.endian);
    replay_vertex.w = 1.0f;
  ...
  if (IsNativeReplay2E01ModelVertexFetch(fetch) && vertex_stride_bytes >= 28) {
    replay_vertex.x = ReadF32(vertex + 0, fetch.endian);
    replay_vertex.y = ReadF32(vertex + 4, fetch.endian);
    replay_vertex.z = ReadF32(vertex + 8, fetch.endian);
    replay_vertex.w = 1.0f;
    const uint32_t packed = ReadU32WithEndian(vertex + 24, fetch.endian);
    constexpr float kInv255 = 1.0f / 255.0f;
    replay_vertex.u = static_cast<float>((packed >> 8) & 0xFFu) * kInv255;
    replay_vertex.v = static_cast<float>((packed >> 16) & 0xFFu) * kInv255;
    return true;
  }
...
void DecodeNativeReplaySharedShaderSkinInputs(const uint8_t* vertex,
  ...
  if (vertex_shader_hash == 0x6B722207E8ECA2B6ull) {
    replay_vertex.has_shared_shader_skin = true;
    return;
  }

  if (vertex_shader_hash == 0x2E01DF902B14A323ull) {
    replay_vertex.has_shared_shader_skin = true;
    return;
  }
  ...
  if (vertex_shader_hash == 0x83BD204594EECAB8ull) {
    if (vertex_stride_bytes < 24) {
      return;
    }
...
std::optional<std::array<float, 4>> EvaluateNativeReplay6B72ModelClip(
    const gpu_replay::ReplayVertex& vertex, const ProjectionCandidate& projection) {
  if (!vertex.has_shared_shader_skin) {
    return std::nullopt;
  }
  ...
  std::array<float, 3> skinned = {};
  for (uint32_t row = 0; row < 3; ++row) {
    const uint32_t constant_index = 15u + row + constant_offset;
    if (constant_index >= projection.shared_shader_constant_present.size() ||
        !projection.shared_shader_constant_present[constant_index]) {
      return std::nullopt;
    }
  ...
  const std::array<float, 3> projected_source = {skinned[2], skinned[0], skinned[1]};
  std::array<float, 4> clip = {};
  for (uint32_t row = 0; row < 4; ++row) {
    const uint32_t constant_index = 11u + row;
    if (constant_index >= projection.shared_shader_constant_present.size() ||
...
std::optional<std::array<float, 4>> EvaluateNativeReplay6E10ModelClip(
    const gpu_replay::ReplayVertex& vertex, const ProjectionCandidate& projection) {
  if (!vertex.has_shared_shader_skin) {
    return std::nullopt;
  }
...
std::optional<std::array<float, 4>> EvaluateNativeReplay83BDModelClip(
    const gpu_replay::ReplayVertex& vertex, const ProjectionCandidate& projection) {
  if (!vertex.has_shared_shader_skin) {
    return std::nullopt;
  }
  ...
  const std::optional<std::array<float, 3>> bone1 =
      evaluate_bone(vertex.shared_shader_constant_offsets[1]);
  const std::optional<std::array<float, 3>> bone2 =
      evaluate_bone(vertex.shared_shader_constant_offsets[2]);
  if (!bone0 || !bone1 || !bone2) {
    return std::nullopt;
  }

  const std::array<float, 3> skinned = {
      (*bone0)[0] * weight0 + (*bone1)[0] * weight1 + (*bone2)[0] * weight2,
      (*bone0)[1] * weight0 + (*bone1)[1] * weight1 + (*bone2)[1] * weight2,
      (*bone0)[2] * weight0 + (*bone1)[2] * weight1 + (*bone2)[2] * weight2,
  };
  const std::array<float, 3> projected_source = {skinned[2], skinned[0], skinned[1]};
  std::array<float, 4> clip = {};
  for (uint32_t row = 0; row < 4; ++row) {
...
std::optional<std::array<float, 4>> EvaluateNativeReplay3094EffectQuadClip(
    const gpu_replay::ReplayVertex& vertex, const ProjectionCandidate& projection) {
  if (!projection.shared_shader_constant_present[0] ||
      !projection.shared_shader_constant_present[255]) {
    return std::nullopt;
  }

  const auto& c0 = projection.shared_shader_constants[0];
  const auto& c255 = projection.shared_shader_constants[255];
  std::array<float, 4> clip = {
      2.0f * (vertex.x + c0[0]) + c255[0],
      -2.0f * (vertex.y + c0[1]) + c255[1],
      vertex.z,
      vertex.w,
  };
...
std::array<float, 4> TransformNativeReplayPositionWithCandidate(
    const gpu_replay::ReplayVertex& vertex, const ProjectionCandidate& projection) {
  if (projection.use_3094_effect_quad) {
    const std::optional<std::array<float, 4>> clip =
        EvaluateNativeReplay3094EffectQuadClip(vertex, projection);
    if (!clip) {
  ...
  if (projection.use_6b72_model_skin) {
    const std::optional<std::array<float, 4>> clip =
        EvaluateNativeReplay6B72ModelClip(vertex, projection);
    if (!clip) {
      const float nan = std::numeric_limits<float>::quiet_NaN();
      return {nan, nan, nan, nan};
    }
  ...
  if (projection.use_2e01_model_skin) {
    const std::optional<std::array<float, 4>> clip =
        EvaluateNativeReplay6B72ModelClip(vertex, projection);
    if (!clip) {
      const float nan = std::numeric_limits<float>::quiet_NaN();
  ...
  if (projection.use_6e10_model_skin) {
    const std::optional<std::array<float, 4>> clip =
  ...
  if (projection.use_83bd_model_skin) {
    const std::optional<std::array<float, 4>> clip =
        EvaluateNativeReplay83BDModelClip(vertex, projection);
    if (!clip) {
      const float nan = std::numeric_limits<float>::quiet_NaN();
      return {nan, nan, nan, nan};
    }
...
ProjectionCandidate FindNativeReplayShaderFinalProjectionCandidate(
  ...
  bool use_b21c_model_skin = false;
  bool use_3094_effect_quad = false;
  bool copy_shader_constants = false;
  uint32_t shared_shader_constant_base = 4;
  const char* source = nullptr;
  switch (event.vertex_shader_hash) {
    case 0x3094A52CE2571823ull:
      constants = {0, 0, 255, 255};
      direct_projection_rows = true;
      use_3094_effect_quad = true;
      copy_shader_constants = true;
      source = "shader-effect-quad-c0-c255";
      break;
    ...
    case 0x6B722207E8ECA2B6ull:
      constants = {11, 12, 13, 14};
      supports_shared_shader_skin = true;
      use_6b72_model_skin = true;
      shared_shader_constant_base = 15;
      source = include_shared_shader_skin ? "shader-model-c15-c17-c11-c14"
                                          : "shader-final-c11-c14";
      break;
    case 0x2E01DF902B14A323ull:
      constants = {11, 12, 13, 14};
      supports_shared_shader_skin = true;
      use_2e01_model_skin = true;
      shared_shader_constant_base = 15;
      source = include_shared_shader_skin ? "shader-model-c15-c17-c11-c14"
                                          : "shader-final-c11-c14";
      break;
    case 0x83BD204594EECAB8ull:
      constants = {11, 12, 13, 14};
      supports_shared_shader_skin = true;
      use_83bd_model_skin = true;
      shared_shader_constant_base = 15;
      source = include_shared_shader_skin ? "shader-weighted-c15-c17-c11-c14"
                                          : "shader-final-c11-c14";
      break;
  ...
  candidate.use_2e01_model_skin = include_shared_shader_skin && use_2e01_model_skin;
  ...
  candidate.use_3094_effect_quad = use_3094_effect_quad;
  ...
  if ((include_shared_shader_skin || copy_shader_constants) &&
      !CopyNativeReplaySharedShaderConstants(event, candidate)) {
    return {};
  }
...
bool IsNativeReplayRestartIndex(uint32_t index, uint32_t index_format,
                                uint32_t restart_index_override =
                                    kNativeReplayNoRestartOverride) {
  if (restart_index_override != kNativeReplayNoRestartOverride &&
      index == restart_index_override) {
    return true;
  }
...
bool ExpandNativeReplayTriangleStripIndices(const std::vector<uint32_t>& strip_indices,
  ...
  for (size_t i = 0; i <= strip_indices.size(); ++i) {
    if (i != strip_indices.size() &&
        !IsNativeReplayRestartIndex(strip_indices[i], index_format, restart_index_override)) {
      continue;
    }
...
uint32_t NativeReplayTriangleStripRestartOverrideForFetch(const VertexFetchSummary& fetch) {
  return (IsNativeReplay6B72ModelVertexFetch(fetch) ||
          IsNativeReplay2E01ModelVertexFetch(fetch) ||
...
bool CanReplayNativeSupportedProjectedTransformDraw(const DrawEvent& event) {
  ...
  const bool ed8d_stride12_projected =
      event.vertex_shader_hash == 0xED8D12865D27DEBFull &&
      event.pixel_shader_hash == 0x7703E4142DFBD4D4ull && event.indexed &&
      event.primitive_type == kXenosPrimitiveTriangleStrip;
  const bool model_stride10_projected =
      event.vertex_shader_hash == 0x6E10B025BC817893ull &&
      event.pixel_shader_hash == 0x1C9617B76D4A368Aull && event.indexed &&
      event.primitive_type == kXenosPrimitiveTriangleStrip;
  const bool weighted_model_stride12_projected =
  ...
  const bool b21c_stride10_projected =
      ...
      event.primitive_type == kXenosPrimitiveTriangleStrip;
  const bool effect_quad_3094_projected =
      event.vertex_shader_hash == 0x3094A52CE2571823ull &&
      event.pixel_shader_hash == 0x969CA710A35A4251ull && !event.indexed &&
      event.primitive_type == kXenosPrimitiveTriangleStrip && event.index_count == 4;
  ...
  const bool model_stride9_projected =
      ...
      event.pixel_shader_hash == 0xD10452A3E31F9C61ull && event.indexed &&
      event.primitive_type == kXenosPrimitiveTriangleStrip;
  const bool model_stride7_projected =
      event.vertex_shader_hash == 0x2E01DF902B14A323ull &&
      event.pixel_shader_hash == 0xD10452A3E31F9C61ull && event.indexed &&
      event.primitive_type == kXenosPrimitiveTriangleStrip;
  ...
  if (ed8d_stride12_projected && !IsNativeReplayED8DSharedSkinVertexFetch(*vertex_fetch)) {
    return false;
  }
  if (model_stride10_projected && !IsNativeReplay6E10ModelVertexFetch(*vertex_fetch)) {
    return false;
  }
  ...
  if (effect_quad_3094_projected && !IsNativeReplay3094EffectVertexFetch(*vertex_fetch)) {
    return false;
  }
  ...
  if (model_stride7_projected && !IsNativeReplay2E01ModelVertexFetch(*vertex_fetch)) {
    return false;
  }
...
class Sidecar final : public EventSink {
 ...
 private:
  ...
  bool CaptureNativeGpuReplayTexturedDraw(const DrawEvent& event,
    ...
    if (event.primitive_type == kXenosPrimitiveTriangleStrip) {
      std::vector<uint32_t> triangle_indices;
      const uint32_t restart_index_override =
          event.indexed ? NativeReplayTriangleStripRestartOverrideForFetch(*vertex_fetch)
                        : kNativeReplayNoRestartOverride;
      const bool expanded = event.indexed
                                ? ExpandNativeReplayTriangleStripIndices(indices, triangle_indices,
                                                                         event.index_format,
                                                                         restart_index_override)
  ...
  bool RejectNativeGpuReplayProjectedTransformGap(const DrawEvent& event, const char* reason,
    ...
    if (native_gpu_replay_projected_reject_log_count_ <
      ...
      char vertex_summary[512] = "none";
      const VertexFetchSummary* reject_fetch = FindDecodableNativeReplayTexturedVertexFetch(event);
      if (!reject_fetch) {
        reject_fetch = FirstUsefulVertexFetch(event);
      }
      if (reject_fetch) {
        const uint32_t attribute_count =
            std::min(reject_fetch->attribute_summary_count,
                     rex::graphics::native_render::kMaxVertexAttributeSummariesPerFetch);
        const auto attribute_at = [&](uint32_t index) -> VertexAttributeSummary {
          return index < attribute_count ? reject_fetch->attributes[index] : VertexAttributeSummary{};
        };
        const VertexAttributeSummary a0 = attribute_at(0);
        const VertexAttributeSummary a1 = attribute_at(1);
        const VertexAttributeSummary a2 = attribute_at(2);
        const VertexAttributeSummary a3 = attribute_at(3);
        std::snprintf(
            vertex_summary, sizeof(vertex_summary),
            "vfetch_c=%u stride_words=%u attrs=%u attr_sig=%#llx "
            "a0=fmt%u@w%d->t%ui%um%uu%us%u a1=fmt%u@w%d->t%ui%um%uu%us%u "
            "a2=fmt%u@w%d->t%ui%um%uu%us%u a3=fmt%u@w%d->t%ui%um%uu%us%u",
            reject_fetch->fetch_constant, reject_fetch->stride_words,
            reject_fetch->attribute_count,
            static_cast<unsigned long long>(TransformGapAttributeSignature(*reject_fetch)),
            a0.data_format, a0.offset_words, a0.result_storage_target,
            a0.result_storage_index, a0.result_write_mask, a0.result_used_components,
            a0.result_swizzle, a1.data_format, a1.offset_words, a1.result_storage_target,
            a1.result_storage_index, a1.result_write_mask, a1.result_used_components,
            a1.result_swizzle, a2.data_format, a2.offset_words, a2.result_storage_target,
            a2.result_storage_index, a2.result_write_mask, a2.result_used_components,
            a2.result_swizzle, a3.data_format, a3.offset_words, a3.result_storage_target,
            a3.result_storage_index, a3.result_write_mask, a3.result_used_components,
            a3.result_swizzle);
      }
      ...
      REXLOG_INFO(
          ...
          "indexed={} index_count={} vfetches={} tfetches={} vertex={} textures={}",
          ...
          event.texture_fetch_summary_count, vertex_summary, texture_summary);
